...
export const ASSETS: Record<AssetId, { label: string; source: AssetSource }> = {
  ...
  // Grace's model is not made yet — same stand-in approach ch4 used for
  // Nikolai: the shared bust placeholder holds the conversation stage until
  // the founder's GLB lands. Drop-in when it does (needs idle + talking clips;
  // eye-tune scale/offset against ch3/ch4 the way the other characters were):
  // 'ch5.character': { label: 'British nursing sister figure', source: { kind: 'glb', url: '/models/ch5-nurse.glb', scale: 1, offset: [0, 0, 0], castShadow: false, clips: { idle: 'Idle_Loop', talking: 'Talking_Loop' } } },
  'ch5.character': { label: 'British nursing sister figure (placeholder)', source: { kind: 'placeholder', component: P.CharacterBust } },
